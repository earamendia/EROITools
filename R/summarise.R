# This script provides functions for EROI calculations and summaries
...
summarise_erois <- function(.tidy_erois_df,
  ...
  # (1) Calculating tidy shares by product at the primary stage:
  ...
  # (2) Calculating tidy shares by product at the final (fuel) stage:
  ...
  # (3) Calculating tidy shares by product at the final (elec) stage:
  ...
  # (4) Calculating tidy shares by product at the final (heat) stage:
  ...
  # (5) Calculating tidy shares by product at the final (fuel+elec+heat) stage:
  ...
  # (6) Merging all shares together:
  tidy_shares_df <- dplyr::bind_rows(
    tidy_shares_primary_df,
...
#'
#' @examples
summarise_useful_stage_erois <- function(.tidy_erois_df,
                                         .tidy_iea_df,
                                         # Whether you want to include non-energy uses products in the EROI calculation
                                         include_non_energy_uses = FALSE,
                                         # Which matrices flows to use for calculating shares
  ...
  # Create a tidy_shares_df first
  summarised_useful_erois <- tidy_shares_df %>%
  ...
  return(summarised_useful_erois)
}
