...
In this vignette, we show how EROIs can be aggregated by fossil fuel group (at the primary, final, and useful stages of energy use), both when using a domestic (here, with Domestic Technology Assumption) and multi-regional perspective (here, with Global Market Assumption).
...
The analyst should provide the final-to-useful efficiencies in a format similar to the `tidy_FU_efficiencies_dta` data frame introduced here. Then, we can bind this data frame to the data frame containing the EROIs calculated with the Input-Output approach:
...
Then, we expand the EROI calculations to the useful stage by applying the final-to-useful efficiencies of each energy product:
